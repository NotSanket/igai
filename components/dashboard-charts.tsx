  ...
  return (
    <DashboardCard className="p-5 sm:p-6">
      ...
      <div className="mt-4 grid items-center gap-2 sm:grid-cols-[1fr_0.9fr]">
        <div className="h-64 min-w-0"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data} dataKey="value" nameKey="name" innerRadius={62} outerRadius={92} paddingAngle={2} stroke="transparent">{data.map((item) => <Cell key={item.name} fill={item.color} />)}</Pie><Tooltip contentStyle={tooltipStyle} formatter={(value) => [`${value}%`, "Requested funding"]} /></PieChart></ResponsiveContainer></div>
...
export function ProposalDistributionChart({ data }: { data: ProposalDistributionDatum[] }) {
  return (
    <DashboardCard className="p-5 sm:p-6">
      ...
      <div className="mt-8 h-64 min-w-0"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ left: -20, right: 4 }}><CartesianGrid vertical={false} stroke="#e8eceb" strokeDasharray="3 4" /><XAxis dataKey="name" tick={{ fill: "#64748b", fontSize: 10 }} axisLine={false} tickLine={false} interval={0} angle={-24} textAnchor="end" height={58} /><YAxis allowDecimals={false} tick={{ fill: "#94a3b8", fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip cursor={{ fill: "#f1f5f4" }} contentStyle={tooltipStyle} /><Bar dataKey="proposals" fill="#0f766e" radius={[5, 5, 0, 0]} maxBarSize={30} /></BarChart></ResponsiveContainer></div>
    </DashboardCard>
  );
}
...
export function DistrictProjectsChart({ data }: { data: DistrictProjectDatum[] }) {
  return (
    <DashboardCard className="p-5 sm:p-6">
      ...
      <div className="mt-6 h-[286px] min-w-0"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} layout="vertical" margin={{ left: 18, right: 20 }}><CartesianGrid horizontal={false} stroke="#e8eceb" strokeDasharray="3 4" /><XAxis type="number" allowDecimals={false} tick={{ fill: "#94a3b8", fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis type="category" dataKey="name" width={92} tick={{ fill: "#475569", fontSize: 11 }} axisLine={false} tickLine={false} /><Tooltip cursor={{ fill: "#f1f5f4" }} contentStyle={tooltipStyle} /><Bar dataKey="projects" fill="#0891b2" radius={[0, 6, 6, 0]} maxBarSize={22} /></BarChart></ResponsiveContainer></div>
    </DashboardCard>
  );
}
